...
export default function ProdukPage() {
  ...
  async function commitInline() {
    ...
    try {
      ...
      showToast(
        `${inlineEdit.field === "stock" ? "Stok" : "Harga"} diperbarui`,
      );
    } catch {
  ...
  async function runBulkCategory() {
    ...
    setProducts((prev) =>
      prev.map((p) => (selected.has(p.id) ? { ...p, category: bulkCat } : p)),
    );
    setSelected(new Set());
    setBulkCat("");
  ...
  async function runBulkPrice() {
    ...
    setProducts((prev) =>
      prev.map((p) => (selected.has(p.id) ? { ...p, price: val } : p)),
    );
    setSelected(new Set());
    setBulkPrice("");
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-gray-50">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100">
                ...
                {["PRODUK", "SKU", "HARGA", "MARGIN", "STOK", "STATUS", ""].map(
                  (h) => (
                    <th
                      key={h}
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((p) => {
                  ...
                  return (
                    <tr
                      key={p.id}
                      className={`hover:bg-gray-50 transition-colors ${selected.has(p.id) ? "bg-amber-50/50" : ""}`}
                    >
                      {/* Checkbox */}
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="flex items-center gap-1.5">
                              ...
                              {p.label && (
                                <span
                                  className={`text-[9px] px-1.5 py-0.5 rounded-full font-medium ${PRODUCT_LABEL_STYLES[p.label] ?? "bg-gray-100 text-gray-600"}`}
                                >
                                  {p.label}
                                </span>
                              )}
                      ...
                      <td className="px-4 py-3">
                        ...
                        inlineEdit.field === "price" ? (
                          ...
                          >
                            <p className="text-sm text-gray-700 group-hover:text-amber-700 transition-colors">
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div className="pb-3 border-b border-gray-100">
                <p className="text-[10px] font-medium text-gray-400 tracking-wider mb-3">
                  WAJIB DIISI
                </p>
                    ...
                    <div>
                      ...
                      {profit !== null && (
                        <p className="text-[10px] text-emerald-600 mt-1">
                          Untung: {formatRupiah(profit)} / item
                        </p>
                      )}
                    </div>
                    <div>
          ...
          >
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex-1 py-2 text-sm text-white bg-red-500 rounded-xl hover:bg-red-600"
              >
                Hapus
              </button>
            </div>
      ...
      {toast && (
        <div
          className={`fixed bottom-6 left-1/2 -translate-x-1/2 text-white text-xs px-4 py-2.5 rounded-full z-50 ${toast.type === "err" ? "bg-red-500" : "bg-gray-900"}`}
        >
          {toast.msg}
        </div>
      )}
