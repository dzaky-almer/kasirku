...
const products: Product[] = [
  { id: "1", name: "Kopi Susu Gula Aren", price: 27000, category: "Kopi", emoji: "☕" },
  { id: "2", name: "Americano Hot", price: 28000, category: "Kopi", emoji: "☕" },
  { id: "3", name: "Cappuccino", price: 32000, category: "Kopi", emoji: "☕" },
  { id: "4", name: "Es Kopi Hitam", price: 21000, category: "Kopi", emoji: "☕" },
  { id: "5", name: "Matcha Latte", price: 35000, category: "Non-Kopi", emoji: "🍵" },
  { id: "6", name: "Coklat Panas", price: 30000, category: "Non-Kopi", emoji: "🍫" },
  { id: "7", name: "Teh Tarik", price: 18000, category: "Non-Kopi", emoji: "🍵" },
  { id: "8", name: "Lemon Tea", price: 20000, category: "Non-Kopi", emoji: "🍋" },
  { id: "9", name: "Croissant", price: 28000, category: "Makanan", emoji: "🥐" },
  { id: "10", name: "Roti Bakar", price: 22000, category: "Makanan", emoji: "🍞" },
  { id: "11", name: "Banana Foster", price: 35000, category: "Makanan", emoji: "🍌" },
  { id: "12", name: "Cheesecake", price: 40000, category: "Makanan", emoji: "🍰" },
];
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden font-sans">
      ...
      <aside className="w-14 bg-white border-r border-gray-100 flex flex-col items-center py-4 gap-1 flex-shrink-0">
        ...
        {[
          { label: "Dashboard", path: "/dashboard", icon: <><rect x="2" y="2" width="5" height="5" rx="1" stroke="currentColor" /><rect x="9" y="2" width="5" height="5" rx="1" stroke="currentColor" /><rect x="2" y="9" width="5" height="5" rx="1" stroke="currentColor" /><rect x="9" y="9" width="5" height="5" rx="1" stroke="currentColor" /></> },
          { label: "Kasir", path: "/kasir", active: true, icon: <><rect x="2" y="4" width="12" height="8" rx="1" stroke="currentColor" /><path d="M8 8v2M6 9h4" stroke="currentColor" /></> },
          { label: "Produk", path: "/produk", icon: <><path d="M8 2L14 5v6L8 14 2 11V5z" stroke="currentColor" /><path d="M8 2v12M2 5l6 3 6-3" stroke="currentColor" /></> },
          { label: "Laporan", path: "/laporan", icon: <><path d="M2 13V6l3-3 3 3 3-2 3 2v7M2 13h12" stroke="currentColor" /></> },
        ].map((nav) => (
          <div
            key={nav.label}
            title={nav.label}
            className={`w-9 h-9 rounded-lg flex items-center justify-center cursor-pointer transition-colors ${
              nav.active ? "bg-amber-50 text-amber-800" : "text-gray-400 hover:text-gray-600 hover:bg-gray-50"
            }`}
          >
            <svg viewBox="0 0 16 16" className="w-4 h-4" fill="none" strokeWidth={1.5}>{nav.icon}</svg>
          </div>
        ))}
        <div className="flex-1" />
        <div className="w-7 h-7 rounded-full bg-amber-100 flex items-center justify-center text-[10px] font-medium text-amber-800">AK</div>
      </aside>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b border-gray-100 px-5 py-3 flex items-center justify-between flex-shrink-0">
          ...
          <span className="text-xs text-gray-400">Kopi Nusantara · Rabu, 2 Apr 2026</span>
        </header>
            ...
            {filtered.map((product) => {
              ...
              return (
                <button
                  ...
                  className={`bg-white border rounded-xl p-4 text-left hover:border-amber-300 transition-all active:scale-95 ${
                    inCart ? "border-amber-400" : "border-gray-100"
                  }`}
                >
                  <div className="text-2xl mb-2">{product.emoji}</div>
                  <p className="text-xs font-medium text-gray-800 leading-tight mb-1">{product.name}</p>
                  <p className="text-xs text-amber-700 font-medium">{formatRupiah(product.price)}</p>
                  {inCart && (
                    <div className="mt-2">
      ...
      <aside className="w-80 bg-white border-l border-gray-100 flex flex-col flex-shrink-0">
        ...
        {cart.length > 0 && (
          <div className="px-5 py-4 border-t border-gray-100">
            ...
            <button
              onClick={handleBayar}
              disabled={cart.length === 0 || paidNum < total}
